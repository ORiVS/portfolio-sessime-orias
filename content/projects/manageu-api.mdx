...
**3) API centrale Django REST** qui porte l’ensemble de la logique métier : comptes, restaurants, menu, marketplace fournisseurs, commandes, évènements, fidélité, etc.
...
**Mobile Android** packagé en **.aab** (format Play Store). 
...
**Interceptors : injection JWT, refresh automatique, retry** 
...
Déployé sur **Vercel**, connecté à l’API centralisée. 
...
+ **UserProfile** pour les infos métier (coordonnées, allergies côté client, infos entreprise côté fournisseur, infos restaurant côté restaurateur). 
...
**Intégrer un composant LLM** configurable (Ollama/HF) pour un chatbot métier sans verrouiller l’architecture. 
